...
/obj/item/weapon/screwdriver/power/attack_self(mob/user)
	playsound(get_turf(user),'modular_boh/sounds/items/tools/change_drill.ogg',50,1)
...
/obj/item/weapon/screwdriver/power/proc/check_sound(atom/A, mob/user)
	if(istype(A, /mob/))
...
/obj/item/weapon/wrench/power/proc/check_sound(atom/A, mob/user)	//Different proc, different subtype and different routine
	if(istype(A, /mob/))
		if(user.a_intent != I_HURT)
			return
	else if(istype(A, /turf/))
		...
		if(istype(A, /obj/machinery/door/airlock/))	//Sound would be called for wrench without being used
			return
	else
		return
	return TRUE
...
/obj/item/weapon/screwdriver/power/resolve_attackby(atom/A, mob/user)
	if(check_sound(A, user))
		if(rand(50))
			playsound(get_turf(user),'modular_boh/sounds/items/tools/drill_use.ogg',50,1)
		else
			playsound(get_turf(user),'modular_boh/sounds/items/tools/drill_use2.ogg',50,1)
	..()
