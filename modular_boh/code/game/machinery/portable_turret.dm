/obj/machinery/porta_turret/ssv
	name = "Skrellian anti-personnel battery"
	desc = "A turret of Skrellian manufacture. Made by Qerr-Kimikruuv Technologies."
	enabled = 1
	ailock = 1
	lethal = 1
	check_synth	 = 0
	check_access = 1
	check_arrest = 0
	check_records = 0
	check_weapons = 0
	check_anomalies = 0
	installation = /obj/item/weapon/gun/energy/pulse_rifle/skrell/vesta
	color = COLOR_DARK_GRAY
	req_access = list("ACCESS_SKRELLSCOUT")

/obj/machinery/porta_turret/mantid
	name = "Mantid Defence Battery"
	desc = "A turret of Mantid manufacture. Complex, expensive, heavy, and surprisingly lethal."
	enabled = 1
	ailock = 1
	lethal = 1
	check_synth	 = 0
	check_access = 1
	check_arrest = 0
	check_records = 0
	check_weapons = 0
	check_anomalies = 0
	installation = /obj/item/weapon/gun/energy/particle
	color = COLOR_PURPLE
	req_access = list("ACCESS_ASCENT")