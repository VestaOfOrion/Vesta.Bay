/obj/machinery/nuclearbomb/station //modularly adds the variable keyturn_recieved to nuclearbomb station
...
/obj/machinery/sd_keyterminal
	...
	icon = 'modular_boh/icon/boh/obj/key_terminal.dmi'
	...
	switch(state)
		...
		if(KEYTERMINAL_STATE_ACTIVE)
			if(!silent)
				...
				playsound(src.loc, 'modular_boh/sounds/effects/alarm7.ogg', 25, 0, 10)
			set_light(0.5, 1, 2, 2, "#fff300")
		if(KEYTERMINAL_STATE_ARMED)
			if(!silent)
				visible_message(SPAN_DANGER("[src] beeps, 'Alert: Key terminal state is now: ARMED.'"))
				playsound(src.loc, 'modular_boh/sounds/effects/siren1b.ogg', 25, 0, 10)
...
/obj/machinery/sd_keyterminal/on_update_icon()
	overlays.Cut()
	overlays += image('modular_boh/icon/boh/obj/key_terminal.dmi', "keyturn_[state]")
	if(key)
		overlays += image('modular_boh/icon/boh/obj/key_terminal.dmi', "keyactive_[key.ownertag]")